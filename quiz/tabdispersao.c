...
int retorna_pontuacao(tabela_dispersao *td, const char *chave)
{
    int index;
    td_elemento * elem;
    ...
    {
        if (!strcmp(elem->obj->chave, chave))
            return elem->obj->pontuacao;
        elem = elem->proximo;
    }
    return -1;
}

void altera_pontuacao(tabela_dispersao *td, const char *chave, int pontuacao)
{
    int index;
    td_elemento * elem;
